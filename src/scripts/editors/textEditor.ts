...
export class TextEditor extends BaseEditor implements IElementEditor {
  ...
  render(focusedElement: ElementData): void {
    ...
    const shadowControls = this.createShadowControls(focusedElement, focusedElement.element.id);
  ...
  private createAlignmentPanel(focusedElement: ElementData): HTMLDivElement {
    ...
    const leftAlignButton = this.createAlignButton(focusedElement, 'left', 'По левому краю');
    const centerAlignButton = this.createAlignButton(focusedElement, 'center', 'По центру');
    const rightAlignButton = this.createAlignButton(focusedElement, 'right', 'По правому краю');
    alignmentPanel.append(leftAlignButton, centerAlignButton, rightAlignButton);
    return alignmentPanel;
  }

  private createAlignButton(focusedElement: ElementData, align: string, title: string): HTMLButtonElement {
  ...
  private createColorPicker(focusedElement: ElementData): { label: HTMLLabelElement, input: HTMLInputElement } {
