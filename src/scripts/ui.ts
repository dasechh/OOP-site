...
class TextEditor implements IElementEditor {
  ...
  private createAlignButton(align: string, title: string): HTMLButtonElement {
    const button = document.createElement('button');
    button.textContent = title;
    button.onclick = () => {
      const element = this.rightPanel.previousElementSibling as HTMLElement;
      if (element) {
  ...
  private createDeleteButton(focusedElement: ElementData): HTMLButtonElement {
    const button = document.createElement('button');
  ...
  private extractShadowValue(shadow: string, type: 'color' | 'offsetX' | 'offsetY' | 'blurRadius'): string | null {
...
class QREditor implements IElementEditor {
  private qrGenerator: QRCodeGenerator;
  private rightPanel: HTMLDivElement;
  private blurValue: number = 0;
